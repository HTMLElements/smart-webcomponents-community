...
Smart('smart-tab-item', class TabItem extends Smart.ContentElement {
    ...
    propertyChangedHandler(propertyName, oldValue, newValue) {
        ...
        switch (propertyName) {
            case 'closeButtonHidden':
                ...
                break;
            case 'disabled': {
                const tabLabelContainer = that.tabLabelContainer;

                if (tabLabelContainer) {
                    newValue ? tabLabelContainer.setAttribute('disabled', '') : tabLabelContainer.removeAttribute('disabled');
                }
            ...
            case 'index':
                ...
                break;
            case 'label': {
                const tabs = that.closest('smart-tabs');

                if (tabs) {
                    tabs._updateTabLabel(that, newValue);
                }
                else {
            ...
            case 'selected':
                if (that.tabLabelContainer) {
                    that.tabLabelContainer.setAttribute('aria-selected', newValue);
                }
    ...
    /**
     * Inserts a child node before another in the smart-tab-item's container.
     *
     * @param {HTMLElement} newNode The new node to append.
     * @param {HTMLElement} node The node to append before.
     */
    insertBefore(newNode, node) {
        ...
        that.firstElementChild.insertBefore(newNode, node);
    }
    ...
    propertyChangedHandler(propertyName, oldValue, newValue) {
        ...
        switch (propertyName) {
            case 'label': {
                ...
                const groupIndex = tabs._groups.indexOf(oldValue);

                if (groupIndex < 0) {
                    break;
                }

                const tabGroupContainer = Array.from(tabs.$.tabStrip.querySelectorAll('.smart-tab-group-container')).
                    find(container => container.group === oldValue);

                Array.from(that.querySelectorAll('smart-tab-item')).forEach(tabItem => tabItem.group = newValue);

                tabGroupContainer.querySelector('.smart-tab-label-text-container').innerHTML = newValue;
                tabGroupContainer.setAttribute('aria-label', newValue);
                tabGroupContainer.group = newValue;

                tabs._groups[groupIndex] = newValue;
                break;
            }
            case 'labelSize':
...
Smart('smart-tabs', class Tabs extends Smart.BaseElement {
    ...
    ensureVisible(index) {
        ...
        if (that.tabPosition === 'top' || that.tabPosition === 'bottom') {
            ...
            if (that.addNewTab && index === that._tabs.length - 1) {
                tabStrip.scrollLeft = tabStrip.scrollWidth - tabStrip.offsetWidth;
            }
        }
    ...
    /**
     * Returns the smart-tab-items inside the 
     * @returns 
     */
    getTabs() {
        const that = this,
        ...
        return Array.from(that.shadowRoot ? that.shadowRoot.querySelectorAll('smart-tab-item') : that.getElementsByTagName('smart-tab-item'));
    }
    ...
    getTabContent(index) {
        const content = this._getTabProperty(index, 'content');
    ...
    getTabLabel(index) {
        return this._getTabProperty(index, 'label');
    }

    _getTabProperty(index, propertyName) {
        const that = this;
    ...
    setTabLabel(index, label) {
        ...
        if (label !== undefined && tab.label !== label) {
            that._updateTabLabel(tab, label);
        }
    }
    ...
    _addTabLabelContainer(tab, addNewTab) {
        ...
        if (tab) {
            tab.disabled ? tabLabelContainer.setAttribute('disabled', '') : tabLabelContainer.removeAttribute('disabled');
        }
    ...
    */
    ...
    _fireDragEvent(event, eventToFire) {
        ...
        that.$.fireEvent(eventToFire, {
            'position': { left: event.pageX, top: event.pageY }, left: event.pageX, top: event.pageY, 'target': target, 'selected': selected, 'index': index, 'label': label
        });
    }
    ...
    _getTabs() {
        ...
        if (that.querySelector('smart-tabs')) {
            tabs = that.shadowRoot ? that.shadowRoot.querySelectorAll('smart-tab-item') : that.getElementsByTagName('smart-tab-item');

            let newTabs = [];
            const content = that.querySelector('.smart-tabs-content-section');

            if (content) {
                for (let i = 0; i < tabs.length; i++) {
                    if (tabs[i].parentNode === content) {
                        newTabs.push(tabs[i])
                    }
                }
    ...
    */
    ...
    _reorderTabs(firstTabIndex, secondTabIndex) {
        ...
        that.$.fireEvent('reorder', { index: secondTabIndex, oldIndex: firstTabIndex, 'newIndex': secondTabIndex, 'originalIndex': firstTabIndex });
    ...
    */
    ...
    _select(index, fireEvent, oldIndex) {
        ...
        if (!tab || tab.disabled || index === oldIndex && ((!that.allowToggle) || (that.allowToggle && that.selectionMode === 'mouseenter'))) {
            that.$.hiddenInput.value = that.selectedIndex;
            return;
        }
     ...
     */
    _updateTabLabel(tab, label) {
        const that = this,
            index = tab.index,
            labelTextContainer = that._tabLabelContainers[index].firstElementChild.children[0],
        ...
        if (tab.tabLabelContainer) {
            tab.tabLabelContainer.setAttribute('aria-label', label);
        }
    }
